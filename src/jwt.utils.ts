...
const recoverPublicKey = (
  ...
  const signatureDecoded = `0x${Buffer.from(signature, "base64").toString(
    "hex"
  )}`;
...
export const jwtEthVerify = (jwt: string): JWTPayload => {
  ...
  try {
    ...
    throw new JwtEthVerifyError(
      `Payload: Failed to decode payload (${(error as Error).message})`
    );
  }
  ...
  if (!isValidAddress) {
    throw new JwtEthVerifyError(
      'Payload: "iss" field must be a valid ethereum address'
    );
  }
  const isChecksumAddress =
    ethers.utils.getAddress(parsedPayload.iss) === parsedPayload.iss;
  if (!isChecksumAddress) {
    throw new JwtEthVerifyError(
      'Payload: "iss" field must be a checksum address'
    );
  }

  if (parsedPayload.iss !== publicKey) {
    throw new JwtEthVerifyError(
      'Payload: "iss" is not the signer of the payload'
    );
  }
